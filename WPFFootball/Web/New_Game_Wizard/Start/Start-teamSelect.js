(function () {
    ...
    function teamSelectCtrl($scope, $stateParams, $uibModal, DB, uiGridConstants) {
        ...
        vm.ViewRoster = function (teamSelected, teamId) {
            ...
            var modalInstance = $uibModal.open({
                ...
                controller: function ($scope, $uibModalInstance, $interval, $filter) {
                    ...
                    $scope.team = JSON.parse(window.Lookup.GetTeam(teamId))[0]; //calls the JSONLookup to return the team name
                ...
                resolve: {
                    roster: function () {
                        return angular.forEach(JSON.parse(window.Lookup.GetRosterPlayers(teamId)), function (value, key) {
                            roster.push({
                                FName: value.FName, LName: value.LName, College: value.College, Age: value.Age,
        ...
        vm.TeamInfo = function (teamSelected, teamId) {
            ...
            var modalInstance = $uibModal.open({
                template: `<div class="modal-header" ng-style="teamPri">
                       ...
                       `,
                resolve: {
                    coaches: coaches.push(JSON.parse(window.Lookup.GetCoaches(teamId))),
                    personnel: personnel.push(JSON.parse(window.Lookup.GetPersonnel(teamId)))
                },
                controller: function ($scope, $uibModalInstance, $interval, $filter, uiGridConstants) {
                    $scope.teamSelected = teamSelected;
                    $scope.team = JSON.parse(window.Lookup.GetTeam(teamId))[0];
                    $scope.teamPri = { 'background-color': $scope.team.MainColor, 'color': $scope.team.TrimColor };
                    $scope.teamSec = { 'background-color': $scope.team.SecondaryColor };
                    $scope.teamTer = { 'background-color': $scope.team.TrimColor };
                    ...
                    $scope.owner = JSON.parse(window.Lookup.GetOwner(teamId))[0];
                    ...
                    ///////////////////////////TEAM INFO SCREEN///////////////////////////////////////////////////////
                    $scope.TeamOffense = JSON.parse(window.Lookup.GetRankTeamOff());
                    ...
                    $scope.pointsDef = _.orderBy($scope.TeamDefense, ['PointsAgainst'], ['asc']);
                    $scope.teamPointsDef = _.filter($scope.TeamDefense, { 'TeamID': teamId })[0].PointsAllowed / 16;
                    $scope.pointsDefRank = _.findIndex($scope.pointsDef, { 'TeamID': teamId }) + 1;
                    $scope.draftPos = DB.getNumEnding($scope.team.DraftPosition);
                    $scope.roster = JSON.parse(window.Lookup.GetRosterPlayers(teamId));
        ...
        /*angular.forEach(DB.load.data.Teams, function (value) {
            vm.sides.push({
                image: value.TeamLogoPath, teamID: value.TeamID, title: value.TeamName + ' ' + value.TeamNickname,
