...
Public Class Personnel
    ...
    Public Function GetSQLString(ByVal PersonnelType As String) As String
        ...
        Select Case PersonnelType
            Case "Personnel"
                result = "PersonnelID int PRIMARY KEY NOT NULL, TeamID int NULL, FName varchar(20) NULL, LName varchar(20) NULL, College varchar(50) NULL, Height int NULL, Weight int NULL, Age int NULL, DOB varchar(12) NULL, PersonnelType int NULL, ScoutRegion varchar(10) NULL, Experience int NULL,
...
DevLB int NULL, DevCB int NULL, DevSF int NULL, LowerBodyTrain int NULL, UpperBodyTrain int NULL, CoreTrain int NULL, PreventInjuryTrain int NULL, StaminaTrain int NULL, " + PersonSQLString
    ...
    End Function
    ''' <summary>
    ''' Generates the number of personnel requested
    ''' </summary>
    ''' <param name="PersonnelNum"></param>
    ''' <param name="XPersonnel"></param>
    ''' <param name="PersonnelDT"></param>
    Public Sub GenPersonnel(ByVal PersonnelNum As Integer, ByVal XPersonnel As Personnel, ByVal PersonnelDT As DataTable)
        ...
        If PersonnelNum = 1 Then 'only does this on the first time through
            For i As Integer = 1 To 33 'initializes the array of dictionaries that keeps track of how many of each personnel type are on each team
        ...
        Try
            ...
            PersonnelDT.Rows(PersonnelNum).Item("ScoutRegion") = GetScoutRegion(PersonnelDT.Rows(PersonnelNum).Item("PersonnelType")) 'sets scout region
            ...
            For x = 1 To 32
                Select Case PersonnelType
                    ...
                    Case 10 'Area Scouts---6 Per Team---Add In Scout Region Code Here--this is being retrofit
                        If PersonnelDict(x).Item(PersonnelType) < 6 Then
                            For i As Integer = 14 To 19 'cycles through to see what area scout they need---this is being retrofit, no the ideal way to handle this
                                If PersonnelDict(x).Item(i) = 0 Then 'if there is no area scout in this region
                                    PersonnelDT.Rows(PersonnelNum).Item("ScoutRegion") = GetScoutRegion(i)
                                    PersonnelDict(x).Item(i) += 1 'signifies they now have a scout for this region
                                End If
                            Next i
                            PersonnelDict(x).Item(PersonnelType) += 1
                            PersonnelDT.Rows(PersonnelNum).Item("TeamID") = x
        ...
        End Try
    End Sub
    Private Function GetScoutRegion(ByVal PersonnelType As Integer) As String
        Dim result As String = ""

        Select Case PersonnelType
            Case 14 : result = "Atlantic"
            Case 15 : result = "Central"
            Case 16 : result = "East"
            Case 17 : result = "Midwest"
            Case 18 : result = "South"
            Case 19 : result = "West"
        End Select
